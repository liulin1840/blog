...
def global_setting(request):
    ...
    scan_list = Article.objects.all().order_by("-click_count")[:6]
    recommend_list = Article.objects.filter(is_recommend =1)[:6]
    # 评论排行
    comment_count_list = Comment.objects.values('article').annotate\
        (comment_count=Count('article')).order_by('-comment_count')
    article_comment_list = [Article.objects.get(pk=comment['article']) \
